...
-------------------------------------------------------------------------
File Description:
View file for the graph depicting moisture sensor data from a plant collection.
   ...
   function(Backbone, CJS, PlantModel, PlantCollection) {
      return Backbone.View.extend({
         ...
         render: function()
         {
             ...
             var moistureVals = new Array(); // replace with an array of moisture values from collection

             // Grab data from collection
             this.collection.each(function(label) {
                lbls.push(label.get('timestamp'));
                moistureVals.push(label.get('moisture'));
            });
            ...
            var ctx = document.getElementById("moistureGraph").getContext("2d");
